...
# distance from A to d (B on d) and u vector of d
def getdistancetoline(A,B,u):
    BA = B-A
    BAxu = np.cross(BA,u)
    dist = np.linalg.norm(BAxu)/np.linalg.norm(u)
    return dist

def getvectorfromangle(A,theta,phi):
    u = np.array([A[0]+ np.sin(theta) * np.cos(phi), A[1]+ np.sin(theta) * np.sin(phi), A[2]*np.sin(theta) ])
    return u


def readpattern(file):
    f = open(file,'r')
    lines = f.readlines()
    phimin = float(lines[2].split()[1])
    phimax = float(lines[3].split()[1])
    stepsphi = float(lines[4].split()[1])
    thetamin = float(lines[7].split()[1])
    thetamax = float(lines[8].split()[1])
    stepstheta = float(lines[9].split()[1])
    deltaphi = (phimax - phimin) / stepsphi
    deltatheta = (thetamax - thetamin) / stepstheta
    a_phi = np.linspace(phimin,phimax,stepsphi +1)
...
#        print len(phigain)
        phigain = l.split()[1:-1]
        np.asarray(phigain,dtype=np.float32)
#        theta = round(thetamin + count,2)
        theta = int(thetamin + count)
#        print theta
        pattern[theta] = phigain
        count+= deltatheta
        a_theta = np.append(a_theta,theta)
    f.close()
#    print 'a_theta in pattern readin: ' , a_theta  
    return pattern

def getpatternaxes(file):
    f = open(file,'r')
    lines = f.readlines()
    phimin = float(lines[2].split()[1])
    phimax = float(lines[3].split()[1])
    ...
    deltatheta = (thetamax - thetamin) / stepstheta
    a_phi = np.linspace(phimin,phimax,stepsphi +1)
    a_theta = np.linspace(thetamin,thetamax,stepstheta +1)
#    print 'a_theta in axis readin: ' , a_theta  
    newa_theta = np.array([])
    for t in a_theta:
#        print int(t)
        newa_theta = np.append(newa_theta,int(t))
    a_theta.astype(np.int)
#    a_theta = np.round(a_theta,2)
#    print 'a_theta in axis readin: ' , newa_theta  
    f.close()
    return [newa_theta,a_phi]
